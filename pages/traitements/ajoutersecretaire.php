        ...
        if(!empty($_POST['nom']) and !empty($_POST['phone']) and !empty($_POST['email']) and !empty($_POST['description'])){
            ...
            if (isset($_FILES['image']) AND $_FILES['image']['error'] == 0) {
                ...
                if (in_array($extension_image, $extensions_permises)){
                    move_uploaded_file($_FILES['image']['tmp_name'], '../../images/secretaires/'.$image.'.'.$extension_image );
                }
            }
            
            $requete="INSERT INTO `secretaire` (`idS`, `nomS`, `descriptionS`, `emailS`, `dateAjout`, `telephoneS`, `etatS`) VALUES (NULL, '$nom', '$des', '$email', '$dat', '$phone', '1')";
